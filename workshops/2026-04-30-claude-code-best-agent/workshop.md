...
### 5. Background Agents - סוכן ששולח סוכן (ודשבורד החלטות)
...
#### השכלול: Dashboard של החלטות קריטיות

סוכן שנתקע על החלטה קריטית ולא יודע מה לעשות - **לא נעצר שקט**. הוא **מציף את השאלה למעלה**, ממשיך לעבוד על מה שהוא יכול, ומחכה לתשובה.

הארכיטקטורה:
- **N סוכנים רצים במקביל** (spawn קלאסי של Claude Code)
- **כל סוכן מחונך להציף החלטות** לדשבורד אחד
- **אתה רואה הכל במקום אחד** - רשימת שאלות + רשימת סטטוסים
- **עונה לשאלה** → הסוכן הרלוונטי ממשיך מיד
- **סנכרון לוואטסאפ** → יכול לענות מהטלפון, מהכביש, מכל מקום

**התוצאה:**
> 5 סוכנים עובדים במקביל. 2 מהם נתקעו בהחלטות. אתה רואה בדשבורד, עונה 2 תשובות בדקה, ו-5 ממשיכים לרוץ.

**זו התקרה של "לופ סגור":** לא אתה מחפש אם מישהו מחכה, הם אומרים לך. לא אתה בודק התקדמות, הדשבורד מציג. **אתה מנהל ציר פנוי לחלוטין, והסוכנים עובדים עבורך.**
...
### 7. הכלי החמישי: Background Agents + Dashboard של החלטות (25 דק')
- "תחקור לי X ברקע" - מפעיל סוכן, ממשיך לעבוד, מקבל תוצאה
- הדגמה חיה של מחקר מקביל - 3 סוכנים ברקע
- **השכלול:** סוכן שנתקע על החלטה קריטית - מציף לדשבורד, לא נעצר שקט
- הצגת הארכיטקטורה: N סוכנים → דשבורד אחד → החלטות נענות במקום אחד

**הדגמה הגמר - peleg-orchestra:**
- WhatsApp group = דשבורד. הצ'אט הוא המערכת.
- הודעה חדשה → סוכן חדש spawn, משיב באותו ערוץ
- Reply להודעת הסוכן → `claude -p --resume` של *בדיוק אותו* סשן
- הודעה קולית → תמלול Groq whisper → מטופלת כטקסט
- **כל ההתחלה-באמצע-סוף קורה בוואטסאפ. בלי UI. בלי dashboard מיוחד.**

**הקפיצה התפיסתית:** *"הדשבורד שאתה מחפש כבר קיים. זה WhatsApp. הצ'אט הוא המערכת."*
...
>
> **"סוכן טוב לא שואל אותך מה לעשות. הוא אומר לך מה הוא צריך שתחליט - ושאר הסוכנים ממשיכים לרוץ."**
