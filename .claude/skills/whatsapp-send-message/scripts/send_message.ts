...
async function main() {
  ...
  if (!args.message && !args.file) {
    ...
    console.error('  npx ts-node send_message.ts --phone "972501234567" --file "/path/to/image.jpg" --caption "Check this!"');
    ...
    console.error("  --file <PATH>     Path to file (image/video/audio/document)");
    console.error("  --caption <TEXT>  Caption for the file (optional)");
  ...
  try {
    ...
    if (args.phone && !args.groupId) {
      ...
      if (args.file) {
        ...
        if (!args.dryRun) {
          const result = await sendFile(chatId, args.file, args.caption);
          console.log(`✓ File sent! ID: ${result.idMessage}`);
    ...
    if (args.groupId && !args.dmAll) {
      ...
      if (args.file) {
        ...
        if (!args.dryRun) {
          const result = await sendFile(chatId, args.file, args.caption);
          console.log(`✓ File sent to group! ID: ${result.idMessage}`);
          console.log(`  URL: ${result.urlFile}`);
        } else {
          console.log("✓ [DRY RUN] Would send file to group");
        }
      }
